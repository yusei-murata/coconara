...
# %%
print('レースID取得中')
race_id_list = race_id_get_year(input_year)
    ...
    for a in range(len(df)):
        if len(df[2][a].split(' ')) >= 2:
            if df[0][a] == '単勝' or df[0][a] == '複勝':
                ...
                for data in range(0,len(df[2][a].split(' '))):
                    if data == 0:
                        columns_list.append(str(df[0][a])+'_'+str(data+1)) #券種の追加
                        columns_list.append(str(df[0][a])+'_'+str(data+1)+'_払い戻し')
...
# %%
df_concat = pd.concat(list, axis=0).reset_index(drop=True)

# %%
df_concat = pd.concat([df_columns, df_concat], axis=0).reset_index(drop=True)

# %%
df_concat.to_excel('results_'+input_year+'.xlsx', index=False)
