...
def excel_prosessing(sheetname):
    ws = wb[sheetname]
    last_row = get_last_row_in_column(ws, 2)
    ...
    df_odds.columns = df_odds.iloc[0]+'_2'
    ...
    for column in range(1,last_column+1):
        if  ws.cell(2, column).value in columns_name_list:
            ...
            list_df.append(pd.concat([df_umaban, df, df_odds], axis=1))
            only_columns_list.append(ws.cell(2, column).value)
    # 上の作成
    up_chart_list = []
    for a, df in enumerate(list_df):
        if a <= 3:
            up_chart_list.append(df.sort_values(by=[df.columns[1],df.columns[2]], ascending=[True,True]).reset_index(drop=True)[['馬番',df.columns[1]]])
        else:
            up_chart_list.append(df.sort_values(by=[df.columns[1],df.columns[2]], ascending=[False,True]).reset_index(drop=True)[['馬番',df.columns[1]]])
    #下の表作成
    down_chart_list = []
    for a, df in enumerate(list_df):
        if a <= 3:
            down_chart_list.append(df.sort_values(by=[df.columns[1],df.columns[2]], ascending=[False,True]).reset_index(drop=True)[['馬番',df.columns[1]]])
        else:
            down_chart_list.append(df.sort_values(by=[df.columns[1],df.columns[2]], ascending=[True,True]).reset_index(drop=True)[['馬番',df.columns[1]]])
