...
def excel_prosessing(sheetname):
    ...
    for a, df in enumerate(list_df):
        if a <= 3:
            ...
            up_chart_list.append(df.sort_values(by=[df.columns[1],df.columns[2]], ascending=[True,False]).reset_index(drop=True)[['馬番',df.columns[1]]])
    #下の表作成
    down_chart_list = []
    for a, df in enumerate(list_df):
        if a <= 3:
            down_chart_list.append(df.sort_values(by=[df.columns[1],df.columns[2]], ascending=[True,False]).reset_index(drop=True)[['馬番',df.columns[1]]])
        else:
