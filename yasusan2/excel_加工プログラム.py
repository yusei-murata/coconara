...
def excel_prosessing(sheetname):
    ws = wb[sheetname]
    last_row = get_last_row_in_column(ws, 10)
    last_row_b = get_last_row_in_column(ws, 2)
    # シートの最終列を取得する
    ...
    for column in range(1,last_column+1):
        if  ws.cell(2, column).value in columns_name_list:
            ...
            list_df.append(pd.concat([df_umaban, df], axis=1))
    ...
    # シートに入力 上表
    for a, column in zip(range(last_column+2, last_column+2 + len(up_chart_df.columns)), range(len(up_chart_df.columns))): #列を指定
        for b, data in zip(range(3, 3 + len(up_chart_df)), range(len(up_chart_df.iloc[:, column])+1)): #行を指定
            ws.cell(b, a).value = up_chart_df.iloc[:, column][data]
            ws.cell(b, a).font = Font(bold=True, name='HGPゴシックE') 
    # シートに入力 下表
    for a, column in zip(range(last_column+2, last_column+2 + len(up_chart_df.columns)), range(len(down_chart_df.columns))): #列を指定
        for b, data in zip(range(len(down_chart_df) + 6, len(down_chart_df) + len(down_chart_df) + 6), range(len(down_chart_df.iloc[:, column]))): #行を指定
            ws.cell(b, a).value = down_chart_df.iloc[:, column][data]
            ws.cell(b, a).font = Font(bold=True, name='HGPゴシックE')
    #列名をシートに入力
    for a, data in zip(range(last_column+2, last_column + 2 + len(up_chart_df.columns)), up_chart_df.columns):
        #上表
        ws.cell(2, a).value = data
        ws.cell(2, a).font = Font(bold=True, name='HGPゴシックE')
        ws.cell(2, a).border = thin_border
        #下表
        ws.cell(len(down_chart_df)+5, a).value = data
        ws.cell(len(down_chart_df)+5, a).font = Font(bold=True, name='HGPゴシックE')
        ws.cell(len(down_chart_df)+5, a).border = thin_border
    ...
    #上の表の色付け
    for a in range(last_column+2, last_column+2 + len(up_chart_df.columns)):
        data = ws.cell(2, a).value
        if data == '馬番':
            for b in range(3, 3 + len(up_chart_df)):
            ...
            for b in range(3, 3 + len(up_chart_df)):
                ...
                try:
                    ...
                    pass
    #下の表色付け
    for a in range(last_column+2, last_column + 2 + len(down_chart_df.columns)):
        data = ws.cell(5 + len(up_chart_df), a).value
        if data == '馬番':
            for b in range(6 + len(up_chart_df), 6 + len(up_chart_df)+ len(down_chart_df)):
                data = ws.cell(b, a).value
                ws.cell(b, a).fill = PatternFill(fill_type='solid', fgColor=fill_color_dict[data])
                ws.cell(b, a).font = Font(color=font_color_dict[data], bold=True, name='HGPゴシックE') #  bold=True 太文字
        else:
    ...
    for a in range(last_column + 2, last_column + 2 + len(up_chart_df.columns)):
            ...
            ws.column_dimensions[col_letter].width = 5  # Set the width to 15 (you can adjust this value as needed)
    # 賞金平均の時には列の幅を変更する
    ...
    # 枠線作成
    for col in range(column_index_from_string('S'), column_index_from_string('AD') + 1):
