...
"""
today = input('日付を入力してください(例:20210101)')
today_year = str(today[:4])
today_month = str(today[4:6])
today_monthz = str(today[4:6]).zfill(2)
today_day = str(today[6:8])
today_dayz = str(today[6:8]).zfill(2)
...
for kaijou in tqdm(kaijou_data_list):
    with pd.ExcelWriter(today_today2+'_'+dic_kaisai[kaijou]+'.xlsx') as writer:
        for url, df_sort, kekka_df, shetname in zip(url_list, shutuba_data_list, kekka_df_list, url_list):
            if kaijou == str(url.split('/')[4][:2]):
                ...
                kekka_df.to_excel(writer, sheet_name=shetname_list[1], index=False)
...
for kaijou in tqdm(kaijou_data_list):
    ...
    wb = openpyxl.load_workbook(today_today2+'_'+dic_kaisai[kaijou]+'.xlsx')
        ...
        for i in range(3, ws.max_row):
            ...
            if data == '1':
                ...
                for q in range(3, ws.max_row):
                    if number == str(ws['C'+str(q)].value):
                        ws['C'+str(q)].fill = fill
        wb.save(today_today2+'_'+dic_kaisai[kaijou]+'.xlsx')
print('完了')
